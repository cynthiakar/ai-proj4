...
class PerceptronModel(object):
    ...
    def run(self, x):
        ...
        """
        "*** YOUR CODE HERE ***"
        # the dot product of weights and the data point x
    ...
    def get_prediction(self, x):
        ...
        run = self.run(x)
         # non-negative for data point x
        if (nn.as_scalar(run) >= 0.0):
            return 1
        else: 
        # negative for data point x
            return -1
    ...
    def train(self, dataset):
        ...
        # then set successful to true and iterate through the dataset
        while True:
            successful = True
            itern = 1
            # for loop to iterate through the data set
            for a, b in dataset.iterate_once(itern):
                # get prediction for a 
                predict = self.get_prediction(a)
                # get the scalar value of b 
                y_value = nn.as_scalar(b)
                # if the example are misclassified then 
                if (y_value != predict):
                    # successful == false 
                    successful = False
                    # update the weights (weights ← weights + direction ⋅ multiplier)
                    nn.Parameter.update(self.w, a, y_value)
            # no misktakes, and the training is 100% accuracy then terminate
            if successful: 
                break
